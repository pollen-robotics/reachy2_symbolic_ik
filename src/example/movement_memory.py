...
from reachy2_symbolic_ik.utils import shoulder_limits_memory
...
def make_line(
    ...
    for i in range(nb_points):
        ...
        if result[0]:
            theta0 = np.linspace(result[1][0], result[1][1], 3)[1]
            is_reachable, theta = shoulder_limits_memory(theta0, result[1], result[2])
            if is_reachable:
